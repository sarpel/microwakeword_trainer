...
from kws_streaming.layers import average_pooling2d, modes
...
class Stream(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        padding_error = (
            "Cell padding must be 'valid'. Additional context: "
        ...
        if not use_one_step and isinstance(wrapped_cell, (tf.keras.layers.Flatten, tf.keras.layers.GlobalMaxPooling2D, tf.keras.layers.GlobalAveragePooling2D)):
            raise ValueError(
        ...
        if isinstance(wrapped_cell, tf.keras.layers.Conv2DTranspose):
            ...
            if self.mode not in (modes.Modes.TRAINING, modes.Modes.NON_STREAM_INFERENCE):
                if padding != "valid":
                    raise ValueError(padding_error)
        ...
        elif isinstance(wrapped_cell, (tf.keras.layers.Flatten, tf.keras.layers.GlobalMaxPooling2D, tf.keras.layers.GlobalAveragePooling2D)):
            # effective kernel size in time dimension
            if self.state_shape:
        ...
        if ring_buffer_size_in_time_dim is not None:
            ...
            logging.warning("ring_buffer_size_in_time_dim overwritten by the passed-in value: %d", ring_buffer_size_in_time_dim)
            self.ring_buffer_size_in_time_dim = ring_buffer_size_in_time_dim

        if self.ring_buffer_size_in_time_dim == 1:
            logging.warning("There is no need to use Stream on time dim with size 1: %s", self.cell.name if hasattr(self.cell, "name") else self.cell)
    ...
    def build(self, input_shape):
        ...
        if isinstance(wrapped_cell, tf.keras.layers.Conv2DTranspose):
            ...
            if self.mode in [modes.Modes.STREAM_INTERNAL_STATE_INFERENCE, modes.Modes.STREAM_EXTERNAL_STATE_INFERENCE]:
                ...
                # channels/filters]
                self.state_shape = [self.inference_batch_size, self.ring_buffer_size_in_time_dim, output_feature_size, filters]
        elif isinstance(
            wrapped_cell,
            (
                tf.keras.layers.Conv1D,
            ...
            self.state_shape = [self.inference_batch_size, self.ring_buffer_size_in_time_dim] + input_shape.as_list()[2:]
        elif isinstance(wrapped_cell, (tf.keras.layers.Flatten, tf.keras.layers.GlobalMaxPooling2D, tf.keras.layers.GlobalAveragePooling2D)) and not self.state_shape:
        ...
        if self.pad_freq_dim == "same" and self.state_shape[2] is not None:
            ...
            # defined by above function: frequency_pad().
        ...
        if self.mode == modes.Modes.STREAM_INTERNAL_STATE_INFERENCE:
            ...
            if self.ring_buffer_size_in_time_dim:
                self.states = self.add_weight(name="states", shape=self.state_shape, trainable=False, initializer=tf.zeros_initializer)
            ...
            if self.ring_buffer_size_in_time_dim:
                self.input_state = tf.keras.layers.Input(shape=self.state_shape[1:], batch_size=self.inference_batch_size, name=self.name + "/" + self.state_name_tag)  # adding names to make it unique
            else:
    ...
    def call(self, inputs):
        ...
        # Create a state variable for streaming inference mode (internal state).
        ...
        if self.pad_freq_dim == "same":
            inputs = frequency_pad(inputs, self.dilation_freq, self.stride_freq, self.kernel_size_freq)
    ...
    def get_input_state(self):
        ...
        if self.mode == modes.Modes.STREAM_EXTERNAL_STATE_INFERENCE:
            ...
            raise ValueError(f"Expected the layer to be in external streaming mode, not `{self.mode}`.")
    ...
    def _streaming_internal_state(self, inputs):
        if isinstance(self.get_core_layer(), tf.keras.layers.Conv2DTranspose):
            ...
            if self.use_one_step:
                # The time dimension always has to equal 1 in streaming mode.
                if inputs.shape[1] != 1:
    ...
    def _streaming_external_state(self, inputs, state):
        state = tf.zeros(self.state_shape, tf.float32) if state is None else state
        if isinstance(self.get_core_layer(), tf.keras.layers.Conv2DTranspose):
            outputs = self.cell(inputs)
    ...
    def _non_streaming(self, inputs):
        ...
        if isinstance(self.get_core_layer(), tf.keras.layers.Conv2DTranspose):
            ...
            if self.pad_time_dim:
                if isinstance(self.cell, (tf.keras.layers.Flatten, tf.keras.layers.GlobalMaxPooling2D, tf.keras.layers.GlobalAveragePooling2D)):
