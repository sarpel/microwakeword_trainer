...
* **Real Live Recordings:** 5–10% (recorded in multiple rooms, including bathrooms/kitchens, using high-quality microphones).
...
**Dequantization formula:** `float_value = (uint8_value - zero_point) * scale`

For this model: `float_value = uint8_value * 0.003906` (zero_point = 0)

**Detection threshold guidance:**
- Wake-word detected when `float_value ≥ threshold` (typical range: 0.5–0.9)
- A uint8 value of 128 → float ≈ 0.5, uint8 value of 230 → float ≈ 0.9
- ESPHome default threshold maps to ~uint8 ≥ 128 (i.e., float ≥ 0.5)
- Use higher threshold (0.7–0.9) to reduce false activations at cost of recall