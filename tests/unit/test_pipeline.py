...
def test_run_success(monkeypatch, capsys) -> None:
    monkeypatch.setattr(pipeline.subprocess, "run", lambda *args, **kwargs: SimpleNamespace(returncode=0))
...
def test_run_failure_exits(monkeypatch) -> None:
    monkeypatch.setattr(pipeline.subprocess, "run", lambda *args, **kwargs: SimpleNamespace(returncode=9))
...
def test_step_train_uses_best_checkpoint_when_present(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(pipeline, "_run", lambda *_args, **_kwargs: SimpleNamespace(returncode=0))
...
def test_step_verify_esphome_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    # exception path
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=2, stdout="", stderr="e"))
    with pytest.raises(SystemExit) as exc:
    ...
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout="noise\n" + json.dumps(payload), stderr=""))
    out = pipeline.step_verify_esphome(tfl)
    assert out["compatible"] is True

    # bad json + nonzero => exit
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=1, stdout="not-json", stderr=""))
    with pytest.raises(SystemExit) as exc2:
        pipeline.step_verify_esphome(tfl)
    assert exc2.value.code == 3

    # compatible false => exit
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout=json.dumps({"compatible": False, "errors": ["x"], "warnings": []}), stderr=""))
...
def test_step_verify_streaming_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    # missing script => skip
    monkeypatch.setattr(pipeline, "Path", lambda p: Path(tmp_path / "missing.py") if p == "scripts/verify_streaming.py" else Path(p))
...
def test_step_verify_streaming_failure_and_success(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=1, stdout="o", stderr="e"))
    with pytest.raises(SystemExit) as exc:
        pipeline.step_verify_streaming(tfl)
    assert exc.value.code == 3

    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout="", stderr=""))
    pipeline.step_verify_streaming(tfl)
...
def test_step_evaluate_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    # missing script
    monkeypatch.setattr(pipeline, "Path", lambda p: Path(tmp_path / "missing.py") if p == "scripts/evaluate_model.py" else Path(p))
...
def test_step_evaluate_parse_and_failure(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(pipeline, "Path", fake_path)
    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout='prefix\n{"fah": 0.1}', stderr=""))
    out = pipeline.step_evaluate(tfl, "standard", "ovr")
    assert out["fah"] == 0.1

    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=1, stdout="", stderr="bad"))
    assert pipeline.step_evaluate(tfl, "standard", None) == {}

    monkeypatch.setattr(pipeline, "_run_capture", lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout="not-json", stderr=""))
    assert pipeline.step_evaluate(tfl, "standard", None) == {}
...
def test_step_gate_variants() -> None:
    ...
    assert pipeline.step_gate({"ambient_false_positives_per_hour": 0.6, "recall_at_target_fah": 0.8}, 0.5, 0.9) is False
...
def test_main_skip_train_requires_checkpoint(monkeypatch) -> None:
    monkeypatch.setattr(pipeline, "create_parser", lambda: SimpleNamespace(parse_args=lambda: SimpleNamespace(skip_train=True, checkpoint=None)))
    with pytest.raises(SystemExit) as exc:
        pipeline.main()
    assert exc.value.code == 1
