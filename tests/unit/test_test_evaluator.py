...
class TestTestEvaluator:
    ...
    def test_basic_metrics_correct(self, tmp_path):
        ...
        # Perfect predictions: y_true=[1,1,0,0], y_score=[0.9,0.8,0.1,0.2]
        def test_factory():
            X = np.random.randn(4, 100, 40).astype(np.float32)
            # Mock model to return specific scores
            with patch.object(model, "predict", return_value=np.array([[0.9], [0.8], [0.1], [0.2]])):
                yield (X, np.array([1, 1, 0, 0], dtype=np.int32), np.ones(4, dtype=np.float32))
    ...
    def test_edge_case_single_class(self, tmp_path):
        ...
        try:
            ...
            # Should work but some metrics may be None
        except Exception:
            pytest.fail("Should handle single class gracefully")
    ...
    def test_numpy_serialization(self, tmp_path):
        ...
        if result:
            ...
            assert "dtype" not in content.lower() or "object" not in content
            # JSON should be parseable
            json.loads(content)

    def test_mcc_computation(self, tmp_path):
        ...
        # Random predictions should be close to 0
        mcc_random = _compute_mcc(5, 5, 5, 5)
        assert abs(mcc_random) < 0.5
