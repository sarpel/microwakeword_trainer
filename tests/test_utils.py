...
from kws_streaming.models import models, utils
from kws_streaming.train import inference


def tf_non_stream_model_accuracy(flags, folder, time_shift_samples=0, weights_name="best_weights", accuracy_name="tf_non_stream_model_accuracy.txt"):
...
def tf_stream_state_internal_model_accuracy(flags, folder, weights_name="best_weights", accuracy_name="tf_stream_state_internal_model_accuracy_sub_set.txt", max_test_samples=1000):
    """Compute accuracy of streamable model with internal state using TF.

    Testing model with batch size 1 can be slow, so accuracy is evaluated
...
def tf_stream_state_external_model_accuracy(flags, folder, weights_name="best_weights", accuracy_name="stream_state_external_model_accuracy_sub_set.txt", reset_state=False, max_test_samples=1000):
...
def tflite_stream_state_external_model_accuracy(flags, folder, tflite_model_name="stream_state_external.tflite", accuracy_name="tflite_stream_state_external_model_accuracy.txt", reset_state=False):
    ...
    logging.info("tflite stream model state external with reset_state %d", reset_state)

    audio_processor = input_data.AudioProcessor(flags)

    set_size = audio_processor.set_size("testing")

    interpreter = tf.lite.Interpreter(model_path=os.path.join(path, tflite_model_name))
...
def tflite_non_stream_model_accuracy(flags, folder, tflite_model_name="non_stream.tflite", accuracy_name="tflite_non_stream_model_accuracy.txt"):
...
def convert_model_tflite(flags, folder, mode, fname, weights_name="best_weights", optimizations=None):
