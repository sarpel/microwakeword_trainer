...
import numpy as np
from kws_streaming.layers import modes, quantize
from kws_streaming.layers.compat import tf, tf1
from kws_streaming.models import model_flags, model_params, model_utils
...
def _clone_model(model, input_tensors):
    ...
    input_tensors, output_tensors, created_layers = tf._keras_internal.engine.functional.reconstruct_from_config(model_config, created_layers=created_layers)  # pylint:disable=protected-access
...
def _copy_weights(new_model, model):
    ...
    def _same_weights(weight, new_weight):
        ...
        return weight.trainable == new_weight.trainable and weight.shape == new_weight.shape and _weight_get_basename(weight) == _weight_get_basename(new_weight)
...
def to_streaming_inference(model_non_stream, flags, mode):
    ...
    if isinstance(model_non_stream.input, (tuple, list)) and len(model_non_stream.input) > 1:
        input_tensors.append(tf.keras.layers.Input(shape=flags.cond_shape, batch_size=1, dtype=model_non_stream.input[1].dtype, name="cond_features"))
        ...
        if len(model_non_stream.input) > 3:
            raise ValueError("Maximum number of inputs supported is 3 (input_audio, cond_features, and cond_audio), but got %d inputs" % len(model_non_stream.input))
...
def model_to_tflite(
    ...
    Returns:
      tflite model
    """
    if sess and mode not in (modes.Modes.STREAM_EXTERNAL_STATE_INFERENCE, modes.Modes.NON_STREAM_INFERENCE):
        raise ValueError("mode %s is not supported in session mode " % mode)

    if not sess and mode not in (modes.Modes.STREAM_INTERNAL_STATE_INFERENCE, modes.Modes.NON_STREAM_INFERENCE):
    ...
    if zero_bias_reduction_steps:
        model_stream = remove_model_zero_bias(model_stream, zero_bias_reduction_steps)

    # Identify custom objects.
    with quantize.quantize_scope():
        if sess:
            # convert Keras inference model to tflite inference model
            converter = tf1.lite.TFLiteConverter.from_session(sess, model_stream.inputs, model_stream.outputs)
        else:
...
def remove_model_zero_bias(
    ...
    if not isinstance(bias, list):
        bias = [bias]
    new_outputs = [out - tf.constant(b) for out, b in zip(model_stream.outputs, bias)]
    model_stream = tf.keras.Model(model_stream.inputs, new_outputs, name=model_stream.name)
    return model_stream
...
def model_to_saved(model_non_stream, flags, save_model_path, mode=modes.Modes.STREAM_INTERNAL_STATE_INFERENCE):
    ...
    if mode not in (modes.Modes.STREAM_INTERNAL_STATE_INFERENCE, modes.Modes.NON_STREAM_INFERENCE):
...
def get_model_with_default_params(model_name, mode=None):
    ...
    if model_name not in model_params.HOTWORD_MODEL_PARAMS:
        raise KeyError(f"Expected 'model_name' to be one of {model_params.HOTWORD_MODEL_PARAMS.keys()} but got '{model_name}'.")
...
def sequential_to_functional(model):
    ...
    input_layer = tf.keras.Input(batch_input_shape=model.layers[0].input_shape[0])
...
def ds_tc_resnet_model_params(use_tf_fft=False):
    ...
    if params.window_stride_samples is None:
        raise ValueError("params.window_stride_samples is None in generating parameters for ds_tc_resnet model.")
    if params.window_size_samples is None:
        raise ValueError("params.window_size_samples is None in generating parameters for ds_tc_resnet model.")
    framing_stride = max(
...
def saved_model_to_tflite(
    ...
    # Identify custom objects.
    with quantize.quantize_scope():
        converter = tf.compat.v2.lite.TFLiteConverter.from_saved_model(saved_model_path)
