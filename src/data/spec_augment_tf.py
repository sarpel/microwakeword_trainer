...
def spec_augment_tf(
    ...
    for i in range(freq_mask_count):
        ...
        freq_mask_size = tf.random.stateless_uniform(
            [], minval=0, maxval=freq_mask_max_size + 1, dtype=tf.int32, seed=seed_tensor
        )
        ...
        freq_mask_start = tf.random.stateless_uniform(
            [], minval=0, maxval=max_start, dtype=tf.int32, seed=seed_tensor2
        )
    ...
    for i in range(time_mask_count):
        ...
        time_mask_size = tf.random.stateless_uniform(
            [], minval=0, maxval=time_mask_max_size + 1, dtype=tf.int32, seed=seed_tensor
        )
        ...
        time_mask_start = tf.random.stateless_uniform(
            [], minval=0, maxval=max_start, dtype=tf.int32, seed=seed_tensor2
        )
...
def batch_spec_augment_tf(
    ...
    for i in range(freq_mask_count):
        ...
        seed1 = tf.constant([seed if seed is not None else i, 0], dtype=tf.int32) if seed is not None else counter + i
        ...
        seed2 = tf.constant([seed if seed is not None else i + 1000, 0], dtype=tf.int32) if seed is not None else counter + i + 1000
