...
class FeatureConfig:
    ...
    def get_frame_count(self, audio_length: int) -> int:
        ...
        Returns:
            Number of frames
        """
        return max(1, int(math.ceil((audio_length - self.window_size_samples) / self.window_step_samples)) + 1)
...
class MicroFrontend:
    ...
    PCAN (Per-Channel Amplitude Normalization) behavior:
        PCAN is hardcoded ON in the pymicro-features C++ backend. There is no
        Python flag to enable/disable PCAN, and the configuration matches the
        ESPHome okay_nabu model by default.
    """
    ...
    def generate_from_file(
        ...
        if target_length is not None:
            if len(audio) > target_length:
                ...
                start = self._rng.randint(0, len(audio) - target_length)  # noqa: S311
