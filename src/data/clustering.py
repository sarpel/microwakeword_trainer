...
def load_embeddings_cache(
    ...
    try:
        ...
        # Validate cache via stored files_hash
        current_hash = _compute_files_hash(audio_paths, model_name)
        try:
            cached_hash = data["files_hash"].tobytes().decode()
        except KeyError:
            logger.debug("Cache miss: legacy format without files_hash")
            return None

        if cached_hash != current_hash:
            logger.debug("Cache miss: files_hash mismatch (files changed or added)")
            return None
