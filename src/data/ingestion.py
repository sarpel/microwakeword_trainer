    ...
    try:
        with wave.open(str(file_path), "rb") as wf:
            ...
            if actual_header_size < 0:
                return False, f"File size ({file_size}) is smaller than the raw audio data ({data_bytes} bytes)"
...
class Clips:
    ...
    def _assign_splits(self, samples: List[SampleRecord]) -> List[SampleRecord]:
        ...
        if self.config.speaker_based_split and any(s.speaker_id for s in samples):
            # Group by speaker — samples without speaker_id use random split
            speakers: dict = {}
            no_speaker_samples: list = []
            for sample in samples:
                if sample.speaker_id:
                    ...
                    no_speaker_samples.append(sample)
            ...
            for sample in samples:
                if sample.speaker_id:
                    if sample.speaker_id in train_speakers:
                        sample.split = Split.TRAIN
                    elif sample.speaker_id in val_speakers:
                        sample.split = Split.VAL
                    else:
                        sample.split = Split.TEST
                    result.append(sample)

            # Assign no-speaker samples via random split to avoid leakage
            rng.shuffle(no_speaker_samples)
            n_no_speaker = len(no_speaker_samples)
            n_ns_train = int(n_no_speaker * self.config.train_split)
            n_ns_val = int(n_no_speaker * self.config.val_split)
            for i, sample in enumerate(no_speaker_samples):
                if i < n_ns_train:
                    sample.split = Split.TRAIN
                elif i < n_ns_train + n_ns_val:
                    sample.split = Split.VAL
                else:
                    sample.split = Split.TEST
                result.append(sample)
