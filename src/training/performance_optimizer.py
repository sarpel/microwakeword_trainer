...
from ..data.dataset import WakeWordDataset
from ..data.tfdata_pipeline import (
    OptimizedDataPipeline,
    benchmark_pipeline,
    create_optimized_dataset,
)
from ..utils.performance import configure_mixed_precision
...
def get_optimal_batch_size(config: dict) -> int:
    ...
    try:
        ...
        try:
            ...
            # Fallback: use TF's current memory allocation
            gpu_memory = tf.config.experimental.get_memory_info(gpus[0].name).get("current", 0)
