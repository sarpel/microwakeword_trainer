...
def step_gate(metrics: dict, target_fah: float, target_recall: float, strict_gate: bool = False) -> bool:
    ...
    if not metrics:
        if strict_gate:
            print("  ✗ No metrics available — quality gate FAILED (strict mode)")
            return False
        print("  ⚠ No metrics available — quality gate SKIPPED (use --strict-gate to fail on missing metrics)")
        return True
    ...
    passed = bool(fah <= target_fah and recall >= target_recall)
    if passed:
