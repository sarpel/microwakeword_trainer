...
def _parse_analysis_output(analysis_text: str, model_content: bytes) -> dict[str, Any]:
    ...
    for tensor_type, shape, dtype in tensors:
        tensor_info = {
            "type": tensor_type,
    ...
    if result["has_quantization"]:
        result["total_parameters"] = model_size_bytes  # ~1 byte per int8 param
    else:
        result["total_parameters"] = model_size_bytes // 4  # 4 bytes per float32 param
