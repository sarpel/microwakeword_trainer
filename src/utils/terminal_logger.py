...
class TeeOutput:
    ...
    def close(self) -> None:
        """Close file streams only (preserve terminal streams)."""
        self._closed = True
        for stream in self.streams:
            # Only close streams that look like real files (have a .name that is a real path)
...
class TerminalLogger:
    ...
    def stop_capture(self) -> None:
        ...
        # Close tee output objects before restoring
        if self._tee_stdout is not None:
            self._tee_stdout._closed = True
        if self._tee_stderr is not None:
            self._tee_stderr._closed = True
